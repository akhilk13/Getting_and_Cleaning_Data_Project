...
*First, all the tables are read into different variables. Then the data from train and test is merged accordingly. 
* Then, only those columns with the mean and standard deviation measures are taken from the whole dataset using the `grep()` function. After extracting these columns, they are given the correct names, taken from `features.txt`.
